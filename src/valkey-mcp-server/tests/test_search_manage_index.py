...
class TestManageIndexList:
    async def test_list_returns_decoded_names(self):
...
class TestManageIndexInfo:
    async def test_info_returns_data(self):
        with patch(f'{MODULE}.ft') as mock_ft:
            mock_ft.info = AsyncMock(return_value={'index_name': 'test'})
            result = await manage_index(action='info', index_name='test')
        assert result['status'] == 'success'
        assert result['info'] == {'index_name': 'test'}

    async def test_info_missing_name(self):
        result = await manage_index(action='info')
        assert result['status'] == 'error'
        assert 'index_name' in result['reason']

    async def test_info_not_found(self):
        with patch(f'{MODULE}.ft') as mock_ft:
            mock_ft.info = AsyncMock(side_effect=RequestError('Unknown index'))
...
class TestManageIndexDrop:
    async def test_drop_success(self):
        with patch(f'{MODULE}.ft') as mock_ft:
            ...
            result = await manage_index(action='drop', index_name='test')
        assert result == {'status': 'success', 'index_name': 'test', 'dropped': True}
...
class TestManageIndexCreate:
    async def test_create_text_field(self):
        with patch(f'{MODULE}.ft') as mock_ft:
            mock_ft.create = AsyncMock()
            result = await manage_index(
                action='create',
                index_name='idx',
                schema=[{'name': 'title', 'type': 'TEXT'}],
            )
        assert result == {'status': 'success', 'index_name': 'idx', 'created': True}
    ...
    async def test_create_missing_schema(self):
        result = await manage_index(action='create', index_name='idx')
        assert result['status'] == 'error'
        assert 'schema' in result['reason']

    async def test_create_missing_field_name(self):
        result = await manage_index(action='create', index_name='idx', schema=[{'type': 'TEXT'}])
        assert result['status'] == 'error'
        assert 'name' in result['reason']

    async def test_create_vector_missing_dimensions(self):
        result = await manage_index(
            action='create', index_name='idx', schema=[{'name': 'v', 'type': 'VECTOR'}]
        )
        assert result['status'] == 'error'
        assert 'dimensions' in result['reason']

    async def test_create_invalid_index_type(self):
        result = await manage_index(
            action='create',
            index_name='idx',
    ...
    async def test_create_invalid_distance_metric(self):
        result = await manage_index(
            action='create',
            index_name='idx',
            schema=[{'name': 'v', 'type': 'VECTOR', 'dimensions': 4}],
