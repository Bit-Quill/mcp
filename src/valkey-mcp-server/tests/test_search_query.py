...
def _make_search_result(docs_dict):
    """Build a GLIDE ft.search result: [count, {b'key': {b'field': b'value'}}]."""
    encoded = {}
...
class TestTextSearch:
    async def test_text_mode_when_no_provider(self):
        with (
            patch(f'{MODULE}._has_provider', return_value=False),
            patch(f'{MODULE}.ft') as mock_ft,
        ):
    ...
    async def test_text_with_filter(self):
        with (
            patch(f'{MODULE}._has_provider', return_value=False),
            patch(f'{MODULE}.ft') as mock_ft,
        ):
            mock_ft.search = AsyncMock(return_value=[0, {}])
            await search(
...
class TestSemanticSearch:
    async def test_semantic_mode(self, mock_provider):
        with (
            patch(f'{MODULE}._has_provider', return_value=True),
            patch(f'{MODULE}._acquire_provider', return_value=mock_provider),
            patch(f'{MODULE}.ft') as mock_ft,
        ):
            mock_ft.search = AsyncMock(
                return_value=_make_search_result(
...
class TestHybridSearch:
    async def test_hybrid_mode(self, mock_provider):
        with (
            patch(f'{MODULE}._has_provider', return_value=True),
            patch(f'{MODULE}._acquire_provider', return_value=mock_provider),
            patch(f'{MODULE}.ft') as mock_ft,
...
class TestFindSimilar:
    async def test_find_similar_mode(self, mock_client):
